...
sys.path.extend(((vendor_path := os.path.join(os.path.dirname(os.path.dirname(
    # fmt: skip
    __file__)), 'setuptools', '_vendor')) not in sys.path) * [vendor_path])
# workaround for #4476
    ...
    def requires(self, extras: Iterable[str] = ()) -> list[Requirement]:
        ...
        for ext in extras:
            try:
                ...
                raise UnknownExtra(
                    f"{self} has no such extra feature {ext!r}") from e
        return deps
