...
class Document:
    ...
    def get_word_before_cursor(
        ...
        start = self.find_start_of_previous_word(
            WORD=WORD, pattern=pattern) or 0
    ...
    def find_boundaries_of_current_word(
        ...
        match_before = get_regex(
            include_leading_whitespace).search(text_before_cursor)
        match_after = get_regex(
            include_trailing_whitespace).search(text_after_cursor)
    ...
    def find_previous_word_ending(
        ...
        text_before_cursor = self.text_after_cursor[:1] + \
            self.text_before_cursor[::-1]
    ...
    def paste_clipboard_data(
        ...
        if data.type == SelectionType.CHARACTERS:
            ...
            if before:
                ...
                lines = self.lines[: l + 1] + \
                    [data.text] * count + self.lines[l + 1:]
