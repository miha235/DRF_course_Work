    ...
    def _stamp_headers(self, visitor_headers=None, append_stamps=False, self_headers=True, **headers):
        ...
        if visitor_headers is not None:
            ...
            _merge_dictionaries(headers, visitor_headers,
                                aggregate_duplicates=append_stamps)
        ...
        if self_headers:
            ...
            _merge_dictionaries(headers, redacted_options,
                                aggregate_duplicates=append_stamps)
            headers["stamped_headers"] = list(set(headers["stamped_headers"]))
...
class group(Signature):
    ...
    def stamp(self, visitor=None, append_stamps=False, **headers):
        ...
        if isinstance(self.tasks, _regen):
            self.tasks.map(_partial(_stamp_regen_task, visitor=visitor,
                           append_stamps=append_stamps, **headers))
        else:
            new_tasks = []
            for task in self.tasks:
    ...
    def _freeze_group_tasks(self, _id=None, group_id=None, chord=None,
        ...
        if isinstance(self.tasks, _regen):
            ...
            results = regen(self._freeze_tasks(
                tasks1, group_id, chord, root_id, parent_id))
...
class _chord(Signature):
    ...
    def stamp(self, visitor=None, append_stamps=False, **headers):
        ...
        if isinstance(tasks, _regen):
            tasks.map(_partial(_stamp_regen_task, visitor=visitor,
                      append_stamps=append_stamps, **headers))
        else:
            stamps = headers.copy()
            for task in tasks:
