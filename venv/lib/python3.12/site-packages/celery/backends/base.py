            ...
            try:
                ...
                if self.always_retry and self.exception_safe_to_retry(exc):
                    if retries < self.max_retries:
                        ...
                        raise_with_context(
                            BackendStoreError(
                                "failed to store result on the backend", task_id=task_id, state=state),
                        )
                else:
                    raise
...
class BaseKeyValueStoreBackend(Backend):
    ...
    def _add_global_keyprefix(self):
        ...
        global_keyprefix = self.app.conf.get(
            'result_backend_transport_options', {}).get("global_keyprefix", None)
        if global_keyprefix:
    ...
    def _store_result(self, task_id, result, state,
        ...
        try:
            self._set_with_state(self.get_key_for_task(
                task_id), self.encode(meta), state)
