...
class HtmlDataGeneration:
    ...
    def data_for_file(self, fr: FileReporter, analysis: Analysis) -> FileData:
        ...
        if self.config.show_contexts:
            contexts_by_lineno = self.data.contexts_by_lineno(
                analysis.filename)
...
HTML_SAFE = string.ascii_letters + \
    string.digits + "!#$%'()*+,-./:;=?@[]^_`{|}~"
...
class HtmlReporter:
    ...
    def __init__(self, cov: Coverage) -> None:
        ...
        self.index_tmpl = Templite(
            read_data("index.html"), self.template_globals)
    ...
    def write_html_page(self, ftr: FileToReport) -> None:
        ...
        context_codes = {y: i for (i, y) in enumerate(
    ...
    def write_file_index_page(self, first_html: str, final_html: str) -> None:
        ...
        print_href = stdout_link(
            index_file, f"file://{os.path.abspath(index_file)}")
