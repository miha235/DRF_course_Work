...
class SqliteDb:
    ...
    def __exit__(self, exc_type, exc_value, traceback) -> None:
        ...
        if self.nest == 0:
            try:
                ...
                if self.debug.should("sql"):
                    self.debug.write(
                        f"EXCEPTION from __exit__: {exc_one_line(exc)}")
                raise DataError(
                    f"Couldn't end data file {self.filename!r}: {exc}") from exc
    ...
    def _execute(self, sql: str, parameters: Iterable[Any]) -> sqlite3.Cursor:
        ...
        try:
            ...
            if self.debug.should("sql"):
                self.debug.write(
                    f"EXCEPTION from execute: {exc_one_line(exc)}")
            raise DataError(
    ...
    def _executemany(self, sql: str, data: list[Any]) -> sqlite3.Cursor:
        ...
        if self.debug.should("sql"):
            ...
            self.debug.write(
                f"Executing many {sql!r} with {len(data)} rows{final}")
            if self.debug.should("sqldata"):
