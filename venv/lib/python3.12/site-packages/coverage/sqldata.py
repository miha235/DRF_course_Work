...
def _locked(method: AnyCallable) -> AnyCallable:
    ...
    def _wrapped(self: CoverageData, *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._debug.should("lock"):
                self._debug.write(
                    f"Locked  {self._lock!r} for {method.__name__}")
...
class CoverageData:
    ...
    * **file tracer names**: the module names of the file tracer plugins that
    ...
    def dumps(self) -> bytes:
        ...
        if self._debug.should("dataio"):
            self._debug.write(
                f"Dumping data from data file {self._filename!r}")
        with self._connect() as con:
            script = con.dump()
            ...
            if lines:
                ...
                for (file, context), numbits in lines.items():
                    with con.execute(
                        ...
                        existing = list(cur)
                    if existing:
                        lines[(file, context)] = numbits_union(
                            numbits, existing[0][0])
    ...
    def erase(self, parallel: bool = False) -> None:
        ...
        if parallel:
            ...
            for filename in glob.glob(pattern):
                if self._debug.should("dataio"):
                    self._debug.write(
                        f"Erasing parallel data file {filename!r}")
