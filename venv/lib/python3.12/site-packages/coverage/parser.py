...
class PythonParser:
    ...
    def __init__(
        ...
        if text is not None:
            ...
            try:
                ...
                raise NoSource(
                    f"No source for code: '{self.filename}': {err}") from err
    ...
    def lines_matching(self, regex: str) -> set[TLineNo]:
        ...
        for match in regex_c.finditer(self.text):
            ...
            matches.update(self._multiline.get(i, i)
                ...
                if node.body:
                    ...
                    if (
                        ...
                        self.raw_docstrings.update(
                            range(first.lineno, cast(
                                int, first.end_lineno) + 1)
                        )
    ...
    def _analyze_ast(self) -> None:
        ...
        assert self._ast_root is not None
        aaa = AstArcAnalyzer(
            self._ast_root, self.raw_statements, self._multiline)
        aaa.analyze()
...
def _make_expression_code_method(noun: str) -> Callable[[AstArcAnalyzer, ast.AST], None]:
    ...
    def _code_object__expression_callable(self: AstArcAnalyzer, node: ast.AST) -> None:
        ...
        self.add_arc(-start, start, None,
                     f"didn't run the {noun} on line {start}")
        self.add_arc(start, -start, None,
...
class AstArcAnalyzer:
    ...
    _code_object__GeneratorExp = _make_expression_code_method(
        "generator expression")
    if env.PYBEHAVIOR.comprehensions_are_functions:
        _code_object__DictComp = _make_expression_code_method(
            "dictionary comprehension")
        _code_object__SetComp = _make_expression_code_method(
            "set comprehension")
        _code_object__ListComp = _make_expression_code_method(
            "list comprehension")
        ...
        if decs:
            ...
            if env.PYBEHAVIOR.trace_decorator_line_again:
                for top, bot in zip(decs, decs[1:]):
                    self.add_arc(self.line_for_node(bot),
                                 self.line_for_node(top))
    ...
    if sys.version_info >= (3, 10):
        def _handle__Match(self, node: ast.Match) -> set[ArcStart]:
            ...
            for case in node.cases:
                ...
                self.add_arc(last_start, case_start,
