...
si_sb_uses_use_case = ("Betelgeuses", "Duses", "Meuses",
                       "Syracuses", "Toulouses")
...
pl_adj_poss_keys = re.compile(
    rf"^({enclose('|'.join(pl_adj_poss))})$", re.IGNORECASE)
...
A_y_cons = re.compile(
    r"^(y(b[lor]|cl[ea]|fere|gg|p[ios]|rou|tt))", re.IGNORECASE)
    ...
    def plural_verb(
        ...
        plural = self.postprocess(
            ...
            self._pl_special_verb(
                word, count) or self._pl_general_verb(word, count),
        )
        return f"{pre}{plural}{post}"
