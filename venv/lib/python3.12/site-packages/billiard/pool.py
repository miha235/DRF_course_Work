    ...
    def _trywaitkill(self, worker):
        ...
        try:
            if os.getpgid(worker.pid) == worker.pid:
                debug(
                    "worker %s is a group leader. It is safe to kill (SIGTERM) the whole group", worker.pid)
        ...
        try:
            if os.getpgid(worker.pid) == worker.pid:
                debug(
                    "worker %s is a group leader. It is safe to kill (SIGKILL) the whole group", worker.pid)
