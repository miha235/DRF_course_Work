...
class CacheProxyConnection(ConnectionInterface):
        ...
        with self._cache_lock:
            ...
            if (
                ...
                return copy.deepcopy(
                    self._cache.get(
                        self._current_command_cache_key).cache_value
                )
    ...
    def _enable_tracking_callback(self, conn: ConnectionInterface) -> None:
        ...
        conn.read_response()
        conn._parser.set_invalidation_push_handler(
            self._on_invalidation_callback)
        ...
        if self.cache is not None:
            return CacheProxyConnection(
                self.connection_class(
                    **self.connection_kwargs), self.cache, self._lock
            )
