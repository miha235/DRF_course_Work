...
def test_Connection_basics_and_content_length() -> None:
    ...
    data = p.send(SERVER, InformationalResponse(
        status_code=100, headers=[]))  # type: ignore[arg-type]
...
def test_chunked() -> None:
    ...
    data = p.send(CLIENT, Data(data=b"1234567890",
                  chunk_start=True, chunk_end=True))
    assert data == b"a\r\n1234567890\r\n"
    data = p.send(CLIENT, Data(
        data=b"abcde", chunk_start=True, chunk_end=True))
    assert data == b"5\r\nabcde\r\n"
    ...
    p.send(
        SERVER, Response(status_code=200, headers=[
                         ("Transfer-Encoding", "chunked")])
    )
...
def test_protocol_switch() -> None:
    for (req, deny, accept) in [
        ...
        for conn in p.conns:
            assert conn.states == {
                CLIENT: SWITCHED_PROTOCOL, SERVER: SWITCHED_PROTOCOL}
...
def test_408_request_timeout() -> None:
    ...
    p = ConnectionPair()
    p.send(SERVER, Response(status_code=408,
