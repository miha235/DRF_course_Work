...
class EndpointEnumerator(_EndpointEnumerator):
    ...
    def get_path_from_regex(self, path_regex):
        if path_regex.endswith(')'):
            logger.warning(
                "url pattern does not end in $ ('%s') - unexpected things might happen", path_regex)
        ...
        for pattern in patterns:
            ...
            if isinstance(pattern, URLPattern):
                ...
                nested_endpoints = self.get_api_endpoints(
                    ...
                    prefix=path_regex,
                    app_name="%s:%s" % (
                        app_name, pattern.app_name) if app_name else pattern.app_name,
                    namespace="%s:%s" % (
                        namespace, pattern.namespace) if namespace else pattern.namespace,
...
class OpenAPISchemaGenerator:
    ...
    def __init__(self, info, version='', url=None, patterns=None, urlconf=None):
        ...
        """
        self._gen = SchemaGenerator(info.title, url, info.get(
            'description', ''), patterns, urlconf)
        self.info = info
        ...
        if url:
            ...
            if parsed_url.path:
                logger.warning(
                    "path component of api base URL %s is ignored; use FORCE_SCRIPT_NAME instead" % url)
            else:
                self._gen.url = url
    ...
    def get_security_requirements(self, security_definitions):
        ...
        if security_requirements is None:
            security_requirements = [{security_scheme: []}
                                     for security_scheme in security_definitions]

        security_requirements = [SwaggerDict._as_odict(
            sr, {}) for sr in security_requirements]
        ...
        if overrides is not None:
            ...
            for method, _ in overrides.items():
                ...
                if view_method is not None:  # pragma: no cover
                    setattr(view_method.__func__,
                            '_swagger_auto_schema', overrides)
