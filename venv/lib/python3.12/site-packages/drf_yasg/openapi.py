...
class SwaggerDict(OrderedDict):
    ...
    def _as_odict(obj, memo):
        ...
        if isinstance(obj, collections_abc.Mapping):
            ...
            for attr, val in items:
                result[SwaggerDict._as_odict(
                    attr, memo)] = SwaggerDict._as_odict(val, memo)
            return result
    ...
    def __reduce__(self):
        ...
        attrs = {k: v for k, v in vars(
            self).items() if not k.startswith('_NP_')}
...
class SchemaRef(_Ref):
    def __init__(self, resolver, schema_name, ignore_unresolved=False):
        ...
        super(SchemaRef, self).__init__(resolver, schema_name,
                                        SCHEMA_DEFINITIONS, Schema, ignore_unresolved)
...
class ReferenceResolver(object):
    ...
    def setdefault(self, name, maker, scope=None):
        ...
        """
        scope = self._check_scope(scope)
        assert callable(maker), "setdefault expects a callable, not %s" % type(
            maker).__name__
        ...
        if ret is None:
            ...
            if value is None:
                ...
                logger.debug(
                    "during setdefault, maker for %s inserted a value and returned a different value", name)
                ret = value
