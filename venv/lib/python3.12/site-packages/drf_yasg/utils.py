...
def swagger_auto_schema(method=None, methods=None, auto_schema=unset, request_body=None, query_serializer=None,
    ...
    def decorator(view_method):
        assert not any(
            hm in extra_overrides for hm in APIView.http_method_names), "HTTP method names not allowed here"
        data = {
        ...
        api_view_http_methods = [m for m in getattr(
            view_cls, 'http_method_names', []) if hasattr(view_cls, m)]
        ...
        if methods or method:
            ...
            assert bool(methods) != bool(
                method), "specify either method or methods"
            ...
            assert all(
                mth in available_http_methods for mth in _methods), "http method not bound to view"
            assert not any(
                mth in existing_data for mth in _methods), "http method defined multiple times"

        if available_http_methods:
            # action or api_view
            assert bool(api_view_http_methods) != bool(
                action_http_methods), "this should never happen"
            ...
            assert not any(
                mth in existing_data for mth in _methods), "swagger_auto_schema applied twice to method"
...
def param_list_to_odict(parameters):
    ...
    """
    result = OrderedDict(((param.name, param.in_), param)
...
def filter_none(obj):
    ...
    if isinstance(obj, dict):
        new_obj = type(obj)((k, v) for k, v in obj.items()
...
def force_serializer_instance(serializer):
    ...
    if inspect.isclass(serializer):
        assert issubclass(
            serializer, serializers.BaseSerializer), "Serializer required, not %s" % serializer.__name__
        return serializer()

    assert isinstance(serializer, serializers.BaseSerializer), \
        "Serializer class or instance required, not %s" % type(
            serializer).__name__
    return serializer
