    ...
    def probe_inspectors(self, inspectors, method_name, obj, initkwargs=None, **kwargs):
        ...
        for inspector in inspectors:
            assert inspect.isclass(
                inspector), "inspector must be a class, not an object"
            assert issubclass(
                inspector, BaseInspector), "inspectors must subclass BaseInspector"

            inspector = inspector(
                self.view, self.path, self.method, self.components, self.request, **initkwargs)
...
class FieldInspector(BaseInspector):
    ...
    def probe_field_inspectors(self, field, swagger_object_type, use_references, **kwargs):
        ...
        return self.probe_inspectors(
            self.field_inspectors, 'field_to_swagger_object', field, {
    ...
    def _get_partial_types(self, field, swagger_object_type, use_references, **kwargs):
        ...
        * a type that should be used for child objects if `field` is of an array type. This can currently have two
        ...
        assert swagger_object_type in (
            openapi.Schema, openapi.Parameter, openapi.Items)
        assert not isinstance(
            field, openapi.SwaggerDict), "passed field is already a SwaggerDict object"
        ...
        description = description if swagger_object_type != openapi.Items else None
...
class ViewInspector(BaseInspector):
    ...
    def _prepend_inspector_overrides(self, inspectors):
        ...
        if extra_inspectors:
            default_inspectors = [insp for insp in getattr(
                self, inspectors) if insp not in extra_inspectors]
    ...
    def get_filter_parameters(self):
        ...
        for filter_backend in getattr(self.view, 'filter_backends'):
            fields += self.probe_inspectors(self.filter_inspectors,
                                            'get_filter_parameters', filter_backend()) or []
    ...
    def serializer_to_parameters(self, serializer, in_):
        ...
        """
        return self.probe_inspectors(
            self.field_inspectors, 'get_request_parameters', serializer, {
                'field_inspectors': self.field_inspectors},
            in_=in_
        ) or []
