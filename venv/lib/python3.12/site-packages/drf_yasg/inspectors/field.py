        ...
        if isinstance(field, (serializers.ListSerializer, serializers.ListField)):
            ...
            if swagger_object_type != openapi.Schema:
                raise SwaggerGenerationError(
                    "cannot instantiate nested serializer as " + swagger_object_type.__name__)
...
def find_regex(regex_field):
    ...
    try:
        ...
        logger.warning('failed to compile regex validator of ' +
                       str(regex_field), exc_info=True)
        return None
...
numeric_fields = (serializers.IntegerField,
                  serializers.FloatField, serializers.DecimalField)
limit_validators = [
    ...
    (validators.MinLengthValidator, serializers.CharField,
     'min_length', operator.__gt__),
    (validators.MaxLengthValidator, serializers.CharField,
     'max_length', operator.__lt__),

    # minItems and maxItems apply to lists
    (validators.MinLengthValidator, (serializers.ListField,
     serializers.ListSerializer), 'min_items', operator.__gt__),
    (validators.MaxLengthValidator, (serializers.ListField,
     serializers.ListSerializer), 'max_items', operator.__lt__),
]
...
def inspect_collection_hint_class(hint_class):
    ...
    child_type_info = get_basic_type_info_from_hint(
        child_class) or {'type': openapi.TYPE_STRING}
        ...
        if getattr(schema, 'properties', {}):
            schema.properties = OrderedDict(
                (self.camelize_string(key), self.camelize_schema(
                    openapi.resolve_ref(val, self.components)) or val)
                for key, val in schema.properties.items()
            )

            if getattr(schema, 'required', []):
                schema.required = [self.camelize_string(
                    p) for p in schema.required]
    ...
    if CamelCaseJSONParser and CamelCaseJSONRenderer:
        def is_camel_case(self):
            return (
                ...
                any(issubclass(renderer, CamelCaseJSONRenderer)
    ...
    class RecursiveFieldInspector(FieldInspector):
        ...
        def field_to_swagger_object(self, field, swagger_object_type, use_references, **kwargs):
            if isinstance(field, RecursiveField) and swagger_object_type == openapi.Schema:
                ...
                assert ref_name is not None, "Can't create RecursiveField schema for inline " + \
                    str(type(proxied))
