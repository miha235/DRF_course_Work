...
class MessageEntity(TelegramObject):
    ...
    __slots__ = ("custom_emoji_id", "language", "length",
                 "offset", "type", "url", "user")
    ...
    def adjust_message_entities_to_utf_16(text: str, entities: _SEM) -> _SEM:
        ...
        positions = sorted(itertools.chain(
            *((x.offset, x.offset + x.length) for x in entities)))
        ...
        for i, position in enumerate(positions):
            ...
            accumulated_length += len(
                text_slice.encode(TextEncoding.UTF_16_LE)) // 2
        ...
        for entity in entities:
            ...
            translated_length = (
                position_translation[entity.offset +
                                     entity.length] - translated_positions
            )
    ...
    def shift_entities(by: Union[str, int], entities: _SEM) -> _SEM:
        ...
        """
        effective_shift = by if isinstance(
            by, int) else len(by.encode("utf-16-le")) // 2
        ...
        *args: Union[Tuple[str, _SEM], Tuple[str, _SEM, bool]],
        ...
        for arg in args:
            ...
            if len(arg) > 2 and arg[2] is True:
                entities = cls.adjust_message_entities_to_utf_16(
                    text, entities)
