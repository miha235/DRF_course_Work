...
class InaccessibleMessage(MaybeInaccessibleMessage):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(chat=chat, message_id=message_id,
                         date=ZERO_DATE, api_kwargs=api_kwargs)
        self._freeze()
...
class Message(MaybeInaccessibleMessage):
    ...
    def __init__(
        ...
        *,
        ...
        with self._unfrozen():
            ...
            self.channel_chat_created: Optional[bool] = bool(
                channel_chat_created)
    ...
    def de_json(cls, data: Optional[JSONDict], bot: Optional["Bot"] = None) -> Optional["Message"]:
        ...
        data["edit_date"] = from_timestamp(
            data.get("edit_date"), tzinfo=loc_tzinfo)
        ...
        data["pinned_message"] = MaybeInaccessibleMessage.de_json(
        ...
        data["users_shared"] = UsersShared.de_json(
            data.get("users_shared"), bot)
        data["chat_shared"] = ChatShared.de_json(data.get("chat_shared"), bot)
        data["chat_background_set"] = ChatBackground.de_json(
            data.get("chat_background_set"), bot)
        data["paid_media"] = PaidMediaInfo.de_json(data.get("paid_media"), bot)
        data["refunded_payment"] = RefundedPayment.de_json(
            data.get("refunded_payment"), bot)
        ...
        data["giveaway_completed"] = GiveawayCompleted.de_json(
            data.get("giveaway_completed"), bot)
        data["giveaway_created"] = GiveawayCreated.de_json(
            data.get("giveaway_created"), bot)
        data["giveaway_winners"] = GiveawayWinners.de_json(
            data.get("giveaway_winners"), bot)
        data["link_preview_options"] = LinkPreviewOptions.de_json(
            data.get("link_preview_options"), bot
        )
        data["external_reply"] = ExternalReplyInfo.de_json(
            data.get("external_reply"), bot)
        data["quote"] = TextQuote.de_json(data.get("quote"), bot)
        data["forward_origin"] = MessageOrigin.de_json(
            data.get("forward_origin"), bot)
        data["reply_to_story"] = Story.de_json(data.get("reply_to_story"), bot)
        data["boost_added"] = ChatBoostAdded.de_json(
            data.get("boost_added"), bot)
        data["sender_business_bot"] = User.de_json(
            data.get("sender_business_bot"), bot)
    ...
    def build_reply_arguments(
        ...
        target_chat_is_self = target_chat_id in (
            None, self.chat_id, f"@{self.chat.username}")
        ...
        quote_position, quote_entities = (
            self.compute_quote_position_and_entities(
                quote, quote_index) if quote else (None, None)
        )
    ...
    async def _parse_quote_arguments(
        ...
        if quote and do_quote:
            raise ValueError(
                "The arguments `quote` and `do_quote` are mutually exclusive")
    ...
    def _parse_html(
        ...
        sorted_entities = sorted(
            entities.items(), key=lambda item: item[0].offset)
        parsed_entities = []
        ...
        # see comment above
        html_text += escape(utf_16_text[last_offset *
                            2:].decode(TextEncoding.UTF_16_LE))
