            ...
            try:
                self._decrypted_secret = self.get_bot().private_key.decrypt(  # type: ignore
                    ...
                    OAEP(mgf=MGF1(algorithm=SHA1()),
                         algorithm=SHA1(), label=None),  # skipcq
                )
    ...
    def decrypted_data(self) -> "Credentials":
        ...
        if self._decrypted_data is None:
            self._decrypted_data = Credentials.de_json(
                decrypt_json(self.decrypted_secret, b64decode(
                    self.hash), b64decode(self.data)),
                self.get_bot(),
            )
...
class SecureValue(TelegramObject):
    ...
    __slots__ = ("data", "files", "front_side",
                 "reverse_side", "selfie", "translation")
