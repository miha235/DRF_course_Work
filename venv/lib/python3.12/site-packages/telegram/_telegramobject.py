...
class TelegramObject:
        ...
        for key, val in state.items():
            try:
                ...
                # 2) The attribute is a private attribute, i.e. it went through case 1) in the past
                    ...
                    # skip adding this to api_kwargs, the attribute is lost forever.
                    continue
    ...
    def __deepcopy__(self: Tele_co, memodict: Dict[int, object]) -> Tele_co:
        ...
        for k in self._get_attrs_names(include_private=True):
            ...
            if k == "api_kwargs":
                ...
                setattr(result, k, MappingProxyType(
                    deepcopy(dict(self.api_kwargs), memodict)))
                continue
    ...
    def _get_attrs_names(self, include_private: bool) -> Iterator[str]:
        ...
        all_slots = (s for c in self.__class__.__mro__[
                     :-1] for s in c.__slots__)  # type: ignore
        # chain the class's slots with the user defined subclass __dict__ (class has no slots)
        all_attrs = (
            chain(all_slots, self.__dict__.keys()) if hasattr(
                self, "__dict__") else all_slots
        )
            ...
            if isinstance(value, (tuple, list)):
                ...
                for item in value:
                    ...
                    # This branch is useful for e.g. Tuple[Tuple[PhotoSize|KeyboardButton]]
                        ...
                        val.append(
                            [
                                i.to_dict(recursive=recursive) if hasattr(
                                    i, "to_dict") else i
                                for i in item
                            ]
                        )
                    # if it's not a TGObject, just append it. E.g. [TGObject, 2]
                    else:
                        val.append(item)
                out[key] = val
