...
_AppType = TypeVar(
    "_AppType", bound="Application")  # pylint: disable=invalid-name
_STOP_SIGNAL = object()
...
if sys.version_info >= (3, 12):
    ...
    _CoroType = Union[Generator["asyncio.Future[object]",
                                None, RT], Awaitable[RT]]
...
class Application(Generic[BT, CCT, UD, CD, BD, JQ], AsyncContextManager["Application"]):
    ...
    async def _update_fetcher(self) -> None:
        try:
            ...
            while not self.update_queue.empty():
                _LOGGER.debug("Dropping pending update: %s",
    ...
    def add_handlers(
        self,
        handlers: Union[
            Union[List[BaseHandler[Any, CCT, Any]],
                  Tuple[BaseHandler[Any, CCT, Any]]],
            Dict[int, Union[List[BaseHandler[Any, CCT, Any]],
                            Tuple[BaseHandler[Any, CCT, Any]]]],
        ],
    ...
    async def __update_persistence(self) -> None:
        ...
        if self.persistence.store_data.bot_data:
            coroutines.add(self.persistence.update_bot_data(
                deepcopy(self.bot_data)))
